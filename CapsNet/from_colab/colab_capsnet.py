...
class Capsule_network(nn.Module):
    def __init__(self, routings):
        ...
        self.digit_capsules=Digit_capsule(routings)#self.digit_capsules=DenseCapsule(6*6*32,8,10,16)
    ...
    def forward(self, x, y = None):
        ...
        if y is None:  # during testing, no label given. create one-hot coding using `length`
            ...
            #if use_cuda:
            #   index = index.cuda()
            y = torch.zeros(length.size()).scatter_(1, index.view(-1, 1).data, 1.)
            y = Variable(y)
...
def reconstruction_loss(x, x_recon):
    ...
    x_recon_reshaped = x_recon.view(x_recon.size(0),28,28)
...
def train(epoch, network, learning_rate, lamda_recon):
    ...
    for j in range(0, epoch):
        ...
        for index, (x, target) in enumerate(train_loader):
            ...
            #print('output size',  output.size())
            # loss =F.nll_loss(output, x) #margin_loss(output,target)
            loss, recon_loss, margin_loss = capsule_loss(output, target, x, reconstruction, lamda_recon)
            # margin_loss.backward()
            loss.backward()
            optimizer.step()
            if index % 10 == 0:
...
def test(model, test_loader, lamda_recon):
    ...
    for index, (x, y) in enumerate(test_loader):
        ...
        loss, recon_loss,margin_loss = capsule_loss(y, y_pred, x, x_recon, lamda_recon)
...
def log_dir_name(learning_rate = 1, num_iterations = 1,lambda_recon = 0.0005):

    # The dir-name for the TensorBoard log-dir.
    s = "./19_logs/lr_{0:.0e}_iterations_{1}_lambda_{2}/"
...
#print('Start!')
#train(50, capsule_network, LEARNING_RATE)
#test_loss, test_acc = test(model=capsule_network, test_loader=test_loader)
#print('test acc = %.4f, test loss = %.5f' % (test_acc, test_loss))
...
dim_num_routings = Integer(low=1, high=4, name='num_routings')

dim_num_epochs = Integer(low=10, high=50, name='num_epochs')

dimensions = [dim_learning_rate,dim_lambda_recon,dim_num_routings]

default_parameters = [1e-3,0.0005,3]
best_accuracy = 0.0
@use_named_args(dimensions=dimensions)
def fitness(learning_rate,lambda_recon,num_routings):
    capsule_network=Capsule_network(num_routings)
    if use_cuda:
      capsule_network.cuda()
    print('Start!')
    print('learning rate: {0:.1e}'.format(learning_rate))
    train(200, capsule_network, learning_rate, lamda_recon = LAMBDA_RECON)
